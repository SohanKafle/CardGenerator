...
@section('content')
    ...
    <div class="max-w-8xl mx-auto p-4 bg-white shadow-lg mt-[7rem] rounded-lg relative z-10">
        ...
        <div class="overflow-x-auto">
            <table id="memberTable" class="min-w-full border-collapse border border-gray-300">
                <thead>
    ...
    <script>
        ...
        document.getElementById('search').addEventListener('input', function() {
            ...
            filterTableByMembername(searchQuery);
        });

        function filterTableByMembername(query) {
            const rows = document.querySelectorAll('#memberTable tbody tr');
            rows.forEach(row => {
                const cells = row.getElementsByTagName('td');
                const memberCell = cells[3];

                if (memberCell) {
                    const memberText = memberCell.textContent.trim().toLowerCase();
